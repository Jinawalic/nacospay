...
function ActivityPageBody() {
  ...
  const handleDownloadReceipt = async () => {
    ...
    try {
      ...
      const rows = [
        ['TRANSACTION ID', receiptTransaction.txnId],
        ['STUDENT NAME', receiptTransaction.student],
        ['MATRIC NUMBER', receiptTransaction.matricNo || 'N/A'],
        ['PAYMENT TYPE', receiptTransaction.typeLabel],
        ['DESCRIPTION', receiptTransaction.details],
        ['AMOUNT PAID', pdfCurrency(receiptTransaction.amount)],
        ['DATE & TIME', new Date(receiptTransaction.dateISO).toLocaleString('en-NG')],
        ['PAYMENT STATUS', receiptTransaction.status.toUpperCase()],
      ] as const;
      ...
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(18);
      const totalText = `TOTAL PAID: ${pdfCurrency(receiptTransaction.amount)}`;
